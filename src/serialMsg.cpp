...
void serial_writelog(const char *format, ...) {
    // 有线(WIRE)模式下, 串口专用于图像帧流(见 cameraClass::sendByUsb),
    // 日志帧混入会破坏 PC 端 App 的 JPEG 帧解析, 故直接静默。
    if(TRANSMIT_MODE_WIRE_E == g_transmitMode){
        return;
    }
        ...
        case SERIAL_MSG_WIFI_CONFIG_E:
            if(len != sizeof(SERIAL_MSG_WIFICONFIG_S)){
                ...
                if(pwlanMsgObj){
                    ...
                    serial_writelog("WIFI_CONFIG ignored: no wlan object (wired mode)\r\n");
                }
            }
            break;
