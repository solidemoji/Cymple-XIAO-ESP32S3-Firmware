...
cameraClass::cameraClass(){
    ...
    if(ucFlags >= FLAG_MAX_E){
        // EEPROM 未初始化(0xFF)或值非法时, 默认设备位置为嘴部(MOUTH)
        // 注: 如需左眼/右眼请分别改为 FLAG_LEFT_EYE_E / FLAG_RIGHT_EYE_E,
        //     或通过串口 POSITION_CFG(type6) 消息配置(会写入 EEPROM 持久保存)
        ucFlags = FLAG_MOUTH_E;
    }
